...
def create_planets():
    ...
    planets.append({
        "position": Vector3(-15, 2, -12),
        "size": 3.5,
        "color": BLUE
    })

    planets.append({
        "position": Vector3(18, 4, 15),
        "size": 5.0,
        "color": ORANGE
    })

    planets.append({
        "position": Vector3(-20, 3, 18),
        "size": 2.5,
        "color": GREEN
    })
...
while not window_should_close():
    ...
    if game_state == MENU:
        ...
        if hovered and is_mouse_button_pressed(
                MOUSE_BUTTON_LEFT):
    ...
    # PLAYING
        ...
        player_position.x = max(
            -world_size,
            min(world_size, player_position.x)
        )

        player_position.z = max(
            -world_size,
            min(world_size, player_position.z)
        )
        ...
        for bullet in bullets[:]:
            ...
            if (
                abs(bullet["position"].x)
                > world_size + 5

                or

                abs(bullet["position"].z)
                > world_size + 5
            ):
